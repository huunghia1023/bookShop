...
const columns = [
  ...
  },
  {
    cell: (row, index, column, id) => (
      <Link to={`/manage-users/${row.id}`} replace>
...
function UserTable() {
  ...
  const addUser = () => {
    navigate("/manage-users/add", { replace: true });
  };
  ...
  async function GetListUser(page, size) {
    try {
      ...
      if (token) {
        ...
        let response = await userResquest.getPaging(page, size, keyword, token);
        if (response.status === 200) {
          ...
          if (!responseData) {
            await Swal.fire({
              icon: "error",
              title: "Error: Can not get list user",
            ...
            <Row className="mt-3">
              <Col xs="6">
                <InputGroup>
                  ...
                  />
                  <Button onClick={GetListUser} color="primary">
                    Search
                  </Button>
                </InputGroup>
              </Col>
              <Col className="position-relative" xs="6"></Col>
            </Row>
          </Container>
